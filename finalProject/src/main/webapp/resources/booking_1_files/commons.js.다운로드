...
var Share = function() {
    ...
    var bindClickEvent = function(option) {
        ...
        $(document).on('click', function(e) {
            ...
            if(	target.parents('#layer_mymega').length == 0
...
$(function() {
    ...
    var selectLoginSession = function() {
        $.ajaxMegaBox({
            ...
            success       : function (data, textStatus, jqXHR) {
                ...
                switch ( isLogin ) {
                    case "Y" :
                        $("#layer_mymega .wrap .login-after").show();
                        $("#layer_mymega .wrap .login-before").hide();
                        // 유저정보 조회
                        setScnBoardInfo();
                        break;
                    default :
                        $("#layer_mymega .wrap .login-after").hide();
                        $("#layer_mymega .wrap .login-before").show();
                        $("#layer_mymega .wrap").show();
                        break;
                }
            },
        });
    ...
    var makeMymegaForCustInfo = function(data){
        ...
        //vip정보
        switch ( data.vipAt ) {
            case "Y" 	: $("#layer_mymega .vip").text( data.custInfo.mbClassNm ); break;
            default		: $("#layer_mymega .vip").text( "" ); break;
        }

        $("#layer_mymega .name").text					( data.custInfo.mbNm );				// 유저명
        $("#layer_mymega .last-date em").text			( data.custInfo.lastLoginDt );		// 마지막 접속일
        ...
        if( data.mbshipJoinInfo.length > 0 ){
            ...
            $.each(data.mbshipJoinInfo, function(i, param) {
                ...
                if(param.svcTyCd == 'SMT01') {
                    html += '<i class="iconset ico-header-film"></i>';
                }

                if(param.svcTyCd == 'SMT02') {
                    html += '<i class="iconset ico-header-classic"></i>';
                }
            });

        }

        $("#layer_mymega .membership").html( html );
    }
    ...
    var makeMymegaForCouponInfo = function(data){
        ...
        // 할인+제휴
        data.cponInfo.cponCnt += data.cponInfo.othcomCponCnt + data.cponInfo.adCponCnt;

        $("#layer_mymega .point .count").text			( data.pointInfo.totHoldPoint.format() );	// 쿠폰포인트
        $("#layer_mymega .coupon .count .cpon").text	( data.cponInfo.cponCnt ); 				// 할인+제휴
        $("#layer_mymega .coupon .count .movie").text	( data.cponInfo.movieMvtchkHoldCnt ); 	// 관람권
    ...
    var makeMymegaForBokdList = function(data){
        ...
        switch ( data.bokdList.length ) {
            case 0 		: $("#layer_mymega .reserve .txt").text( "예매내역이 없어요!" ); break;
            default		: $("#layer_mymega .reserve .txt").text( gfn_charToHtml(data.bokdList[0].movieNm) ); break;
        }

        //$("#layer_mymega .reserve .btn-fixed em").text	( data.bokdList.length );				// 예매내역 건수
    }

    //구매내역
    var makeMymegaForPurcList = function(data){

        $("#layer_mymega .buy .count em").text			( data.pureInfo.length );				// 구매건수
            ...
            success       : function (data, textStatus, jqXHR) {
                ...
                if(data.mbNoticeList != undefined && data.mbNoticeList.length > 0) {
                    ...
                    for(var i in data.mbNoticeList) {
                        ...
                        if(rowData.notiLinkUrl.split("/")[1] == "mobile-order"){
    ...
    $('.container').on('click','.eventBtn', function(e) {
        ...
        if( $(this).data('netfunnel') == 'Y' ){
            //넷퍼넬 적용
            form.attr('action', '/event/detail?eventNo='+$(this).data('no'));
